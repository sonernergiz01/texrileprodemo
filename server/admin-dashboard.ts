...
adminDashboardRouter.get("/order-summary", async (req: Request, res: Response) => {
  try {
    // Aktif sipariş sayısı, toplam miktar
    ...
    // Sabit değerler
    const monthlySales = 1548000;
    const growthRate = 12.8;
    const avgProductionTime = 12.5;
...
adminDashboardRouter.get("/sales-orders", async (req: Request, res: Response) => {
  try {
    ...
    // Aylık satış verileri (sabit veri)
    const monthlySalesData = [
      { name: "Oca", sales: 950.2 },
      { name: "Şub", sales: 875.5 },
      { name: "Mar", sales: 1025.8 },
      { name: "Nis", sales: 1130.4 },
      { name: "May", sales: 1280.3 },
      { name: "Haz", sales: 1548.1 },
      { name: "Tem", sales: 1420.6 },
      { name: "Ağu", sales: 1350.9 },
      { name: "Eyl", sales: 1480.2 },
      { name: "Eki", sales: 1520.5 },
      { name: "Kas", sales: 1640.8 },
      { name: "Ara", sales: 1780.3 }
    ];
...
adminDashboardRouter.get("/production-plans", async (req: Request, res: Response) => {
  try {
    // Üretim plan durumlarını Türkçe durumları sayarak hesaplayalım
    const productionStats = await db.execute(sql`
      SELECT 
        COUNT(CASE WHEN status = 'Beklemede' THEN 1 END) as pending,
        COUNT(CASE WHEN status = 'Devam Ediyor' THEN 1 END) as inProgress,
        COUNT(CASE WHEN status = 'Tamamlandı' THEN 1 END) as completed,
        COUNT(CASE WHEN status = 'Gecikmiş' THEN 1 END) as delayed
      FROM production_plans
      WHERE created_at >= NOW() - INTERVAL '90 days'
    `);
    
    // Eklediğimiz production_steps verileriyle departman bazlı istatistikleri çekelim
...
adminDashboardRouter.get("/machine-queues", async (req: Request, res: Response) => {
  try {
    // Makine verilerini veritabanından çekelim
    const machineStats = await db.execute(sql`
      SELECT 
        ...
        END as doluluk
      FROM machines
      WHERE name IS NOT NULL AND department_id = 11
      ORDER BY id DESC
      LIMIT 6
    `);
    
    // Veritabanında makine verisi varsa kullanalım
    if (machineStats && machineStats.length > 0) {
      return res.json(machineStats);
    }
    
    // Makine verisi yoksa statik veri kullanalım
    const machines = [
...
adminDashboardRouter.get("/quality-reports", async (req: Request, res: Response) => {
  try {
    // Kalite değerlendirmeleri (sabit veri)
    ...
    // Hata tipleri dağılımı (sabit veri)
    const defectTypes = [
...
adminDashboardRouter.get("/sales-trends", async (req: Request, res: Response) => {
  try {
    // Son 12 aydaki trend (sabit veri)
    const salesTrends = [
